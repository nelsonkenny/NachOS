...
void professor_student_synch_test();//Function to test the professor student synchronization problem;
void answer_start();//Function that puts the ask semaphore to sleep, so no student can ask a question while the professor is answering a question
void answer_done(); //Function that wake the ask semaphore up indicating that a student can now ask a question
void question_start(int); //Function that puts the answer semaphore to sleep, so the professor can wait until the student is done to answer a question
void question_done(int);//Function that wakes the answer semaphore up indicating the professor can now answer a question
void student_function(int); //Function that iterates calls of answer start and answer done
void professor_function();//Function that iterates calls of question start and question done. 

void producer_consumer_test();//Function to test the Producer Consumer Bounded Buffer problem
void producer(int); //Function that allows the producer to add string characters into the buffer
void consumer(int); //Function that allows the consumer to consume each item added in the buffer
#endif // SYNCH_H
